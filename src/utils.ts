...
export function getChainsWithDefault(chains: number[], defaultChainId: number) {
  ...
  if (idx === -1) {
    throw new Error(
      `Invalid chainId ${defaultChainId}. Make sure to include it in the "chains" array.`
    );
  }
 ...
 */
 export async function legacyGetBestUrl(urls: string[], timeout: number): Promise<string> {
  // if we only have 1 url, it's the best!
  if (urls.length === 1) return urls[0]
