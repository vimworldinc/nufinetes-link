...
export type NufinetesExtendWeb3Provider = Web3Provider & {
  INufinetesProvider: EthereumProvider
  wcInstance: WalletConnect
}

class NufinetesProvider extends EthereumProvider {
...
class NufinetesWeb3Provider extends Web3Provider {
  readonly INufinetesProvider!: EthereumProvider
  readonly wcInstance!: WalletConnect
  ...
  constructor(provider: WalletConnect, opt: any) {
    ...
    const INufinetesProvider: EthereumProvider = new NufinetesProvider(provider, opt)
        ...
        if (prop === 'on' || prop === 'off') {
          const proxiedFunction = (
            ...
            if (
              ...
              return target.wcInstance[prop].call(target.wcInstance, method as WalletConnectEvents, callback)
            }
            return target[prop].call(target, method, callback)
          }
          return proxiedFunction
        } else if (target.wcInstance && prop in target.wcInstance && prop !== 'constructor' && prop !== 'prototype') {
