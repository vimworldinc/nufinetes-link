...
import type WalletConnectProvider from "./legacy-ethereum-provider";
...
import { legacyGetBestUrl } from "./utils";

const URI_AVAILABLE = "URI_AVAILABLE";
 ...
 */
export interface LegacyWalletConnectConstructorArgs {
  actions: Actions;
...
export class LegacyNufinetesConnector extends Connector {
  ...
  constructor({
    ...
    timeout = 5000,
    onError,
  }: LegacyWalletConnectConstructorArgs) {
  ...
  private async isomorphicInitialize(
    ...
    const rpc = Promise.all(
      Object.keys(this.rpc).map(
        async (chainId): Promise<[number, string]> => [
          Number(chainId),
          await legacyGetBestUrl(this.rpc[Number(chainId)], this.timeout),
        ]
      )
    ).then((results) =>
    ...
    return (this.eagerConnection = import("./legacy-ethereum-provider").then(
      async (m) => {
  ...
  public async activate(desiredChainId?: number): Promise<void> {
    ...
    try {
      ...
      const chainId = parseChainId(
        await this.provider.request<string>({ method: "eth_chainId" })
      );
      /**
