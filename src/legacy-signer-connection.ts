...
export class SignerConnection extends IJsonRpcConnection {
  ...
  public async send(payload: any) {
    ...
    this.sendPayload(payload)
      ...
      .catch((e) =>
        this.events.emit("payload", formatJsonRpcError(payload.id, e.message))
      );
  }
  ...
  private register(opts?: IWCEthRpcConnectionOptions): IConnector {
    ...
    this.chainId =
      typeof opts?.chainId !== "undefined" ? opts.chainId : this.chainId;
    ...
    this.wc =
      typeof opts?.connector !== "undefined"
        ? opts.connector
        : new WalletConnect(connectorOpts);
